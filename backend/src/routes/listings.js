...
router.get("/", async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Could not load listings", details: err.message });
  }
});
...
router.get("/stats", async (_req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Could not load listing stats", details: err.message });
  }
});
...
router.post("/", requireAuth, requireRole("host", "admin"), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Could not create listing", details: err.message });
  }
});
...
router.patch("/:id/availability", requireAuth, requireRole("host", "admin"), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Could not update availability", details: err.message });
  }
});
